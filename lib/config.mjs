...
export async function resolveRuntime(args = [], options = {}) {
  ...
  const bridgeDir = path.resolve(
    readArg(args, '--bridge-dir') ||
      ...
      config.bridgeDir ||
      path.join(os.homedir(), '.agent-bridge', 'workspaces', workspaceName || path.basename(projectRoot))
  );
  const controlPath = path.join(bridgeDir, 'control.json');
  return {
    workspaceName: workspaceName || workspaceConfig.id || config.id || path.basename(projectRoot),
    configPath,
    projectRoot,
    bridgeDir,
    controlPath
