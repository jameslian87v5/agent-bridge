...
export const dirs = ['queue', 'inflight', 'done', 'acks', 'reviews', 'artifacts', 'logs', 'failed', 'threads'];
...
export async function ensureThread(bridgeDir, threadId, type = 'ad-hoc') {
  const threadPath = path.join(bridgeDir, 'threads', `${threadId}.json`);
  if (existsSync(threadPath)) return await readJson(threadPath, {});
  const thread = {
    threadId,
    type,
    status: 'in-progress',
...
export async function updateThreadStatus(bridgeDir, threadId, status) {
  if (!threadId) return;
  const threadPath = path.join(bridgeDir, 'threads', `${threadId}.json`);
  if (!existsSync(threadPath)) return;
  const thread = await readJson(threadPath, null);
  if (!thread) return;
  thread.status = status;
  await writeJson(threadPath, thread);
}

export async function buildThreadHint(bridgeDir, threadId) {
  if (!threadId) return '';
  const threadPath = path.join(bridgeDir, 'threads', `${threadId}.json`);
  const thread = await readJson(threadPath, null);
  if (!thread) return '';
  const lines = [`This event is part of thread "${threadId}" (type: ${thread.type}, status: ${thread.status}).`];
  lines.push(`Read ${threadPath} for thread context.`);
  if (thread.type === 'plan-execution') {
    lines.push('This is a multi-stage task. Follow the plan stages in order. Do not skip ahead. Do not stop mid-plan.');
    if (thread.status === 'in-progress' || thread.status === 'waiting-review') {
      lines.push('The thread is NOT done. After reading the review, you MUST either send a new event for the next stage or explicitly mark the thread done. Do not just acknowledge and stop.');
    }
  } else if (thread.type === 'discussion') {
    lines.push('This is an open discussion. Iterate until convergence, then mark the thread done.');
  } else if (thread.type === 'review') {
    lines.push('This is a single-round review. Complete your review and close the thread.');
  }
  lines.push('');
  return lines.join('\n') + '\n';
}
